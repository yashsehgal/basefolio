...
import { fetchAllPastHackathons, fetchAllUpcomingHackathons, fetchHackathonCities } from "@/middleware";
            ...
            return (
              ...
              >
                <Button
                  variant={"solid"}
                  size="large"
                  className={cn("justify-between p-6 bg-white focus:shadow-sm focus:scale-105 focus:outline-none")}
                  stretch
                >
                  {location}
                </Button>
...
const PastHackathonsActionView: React.FunctionComponent = () => {
  const [pastHackathons, setPastHackathons] = useState<Array<{
    title: string;
    hackathonSlug: string;
  }>>([]);
  const [filteredPastHackathons, setFilteredPastHackathons] = useState<Array<{
    title: string;
    hackathonSlug: string;
  }>>([]);
...
const UpcomingHackathonsActionView: React.FunctionComponent = () => {
  const [upcomingHackathons, setUpcomingHackathons] = useState<Array<{
    title: string;
    hackathonSlug: string;
  }>>([]);
  const [filteredUpcomingHackathons, setFilteredUpcomingHackathons] = useState<Array<{
    title: string;
    hackathonSlug: string;
  }>>([]);
