...
const ProfileNavigationOptionsData: Array<{ title: string; value: ProfileViewTabsType; icon: React.ReactNode }> = [
  {
    title: "About",
    value: "about",
...
const ProfileNavigation: React.FunctionComponent<ProfileNavigationProps> = ({
  ...
  return (
    <nav className={cn("profile-navigation h-[100svh] border-r",
      ...
      className)} {...props}>
      <div className={cn("profile-options-wrapper grid grid-cols-1 w-full gap-4",
        ...
        {ProfileNavigationOptionsData.map((option, index) => {
          return (
            ...
            >
              ...
              <span className="profile-option-title-wrapper hidden md:block">{option.title}</span>
            </Button>
          )
        })}
      </div>
