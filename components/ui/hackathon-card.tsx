...
const HackathonCard: React.FunctionComponent<
  React.HTMLAttributes<HTMLDivElement> & HackathonInterface
> = (hackathonData): React.ReactNode => {
  return (
    <CardContainer
      className={cn(
        "hackathon-card flex flex-col items-start justify-between",
        hackathonData.className,
      )}
    >
      <div className="hackathon-card-body">
        {hackathonData.isFeatured && (
      ...
      <Link
        href={`${APP_BASE_HOSTNAME}/hackathons/${hackathonData.slug}`}
        className="w-full"
      >
