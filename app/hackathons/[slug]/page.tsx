...
const HackathonMicrosite: React.FunctionComponent = () => {
  ...
  const [micrositeTab, setMicrositeTab] = useState<HackathonMicrositeTabType>("overview");
...
const HackathonMicrositeTabNavigation: React.FunctionComponent<HackathonMicrositeTabNavigationProps>
  = ({ micrositeTab, setMicrositeTab }) => {
    return (
      <div className={cn("sub-navigation-actions-wrapper border flex flex-row items-center justify-center w-full bg-white p-2 rounded-2xl max-sm:grid max-sm:grid-cols-2 max-sm:gap-2")}>
        {HackathonMicrositeTabs.map((option, index) => {
          return (
            <Button
              key={index}
              variant={(micrositeTab === option.value) ? "primary" : "secondary"}
              className={cn(micrositeTab !== option.value && "shadow-none hover:shadow-none", "uppercase text-sm")}
