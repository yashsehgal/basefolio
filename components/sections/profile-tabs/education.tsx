'use client';
import { Button, CardContainer, FormItemWrapper, Input, Label } from "@/components/ui"
import { UserAuthenticationContext } from "@/contexts";
import { cn } from "@/helpers";
import { AuthorizedUserEducationOperations, fetchUserEducation } from "@/middleware";
...
const EducationTab: React.FunctionComponent = () => {
  const [education, setEducation] = useState<Array<AuthorizedUserEducationType>>([]);

  const { userData } = useContext(UserAuthenticationContext);

  const [notFound, setNotFound] = useState<boolean>(false);

  const [newEducationInput, setNewEducationInput] = useState<AuthorizedUserEducationType>({
    course: "",
    instituteName: "",
    description: ""
  });
  ...
  return (
    <div className="education-tab-content-container">
      <CardContainer
        ...
        mainAction={
          !addNewEducation && <Button
            onClick={() => setAddNewEducation(true)}
          >
            Add education
          </Button>
        }
      >
        {addNewEducation && <NewEducationInputForm
        ...
        </div> : <EmptyState icon={School}>
          {EMPTY_STATE_CONTENT.EDUCATION}
        </EmptyState>}
      </CardContainer>
    </div>
  )
  ...
  return (
    <div className="education-row-card pt-4 pb-6 border-b border-b-zinc-100 grid grid-cols-1 gap-4">
      {!editView && <div>
        <div className="flex flex-row items-start justify-between">
          <div className="flex flex-col items-start gap-0.5">
            ...
            <p className="text-zinc-400 text-sm">{"from, " + instituteName}</p>
          </div>
          <div className="flex flex-row items-center justify-end gap-2">
            {!editView && <Button variant="solid" size="small" className="p-2" onClick={() => {
              setEditView(true);
            }}>
      ...
      {editView && <>
        ...
        {editView && <div className="flex flex-row items-center justify-between">
